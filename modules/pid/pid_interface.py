...
class PIDInterface:
    def __init__(self, shared_memory_object):
        ...
        kp_row = self.get_env_array("KP_ROW", [3000, 3000, 2500, 30, 2, 2])
        ki_row = self.get_env_array("KI_ROW", [0.5, 0.5, 2, 2, 0.5, 0.5])
        kd_row = self.get_env_array("KD_ROW", [0.1, 0.1, 0.1, 0.2, 0.5, 0.5])
    ...
    def get_env_array(self, name: str, default: list[float]) -> np.ndarray:
        value = os.environ.get(name)

        if value is None:
            return np.array(default, dtype=float)

        return np.array([float(x.strip()) for x in value.split(",")], dtype=float)
